...
async function run() {
  try {
    ...
    app.get("/category/:id", async (req, res) => {
      const id = req.params.id;
      console.log(req.params.id);
      const product_category = await categoriesCollection
    ...
    app.get("/products/:id", async (req, res) => {
      const id = req.params.id;
      const product_category = await productsCollection
        .find({ category_id: id })
        .toArray();
      res.send(product_category);
    });
    ...
    //get bookingproduct accoding to id
    app.get("/bookings/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: ObjectId(id) };
      const booking = await bookingsCollection.findOne(query);
    ...
    //is Admin or not check by user email?
    app.get("/users/admin/:email", async (req, res) => {
      const email = req.params.email;
      const query = { email };
      const user = await usersCollection.findOne(query);
    ...
    //is seller or not check by user email?
    app.get("/users/seller/:email", async (req, res) => {
      const email = req.params.email;
      const query = { email };
      const user = await usersCollection.findOne(query);
    ...
    //is buyer or not check by user email
    app.get("/users/buyer/:email", async (req, res) => {
      const email = req.params.email;
      const query = { email };
      const user = await usersCollection.findOne(query);
    ...
    // //get all user
    app.get("/users", async (req, res) => {
      const userCategory = req.query.userType;
      const query = { role: userCategory };
      const users = await usersCollection.find(query).toArray();
      res.send(users);
    });
    //email wish user get
    ...
    app.get("/addProducts/:role", async (req, res) => {
      ...
      console.log(role);
      const query = { role: role };
      const proCollection = await productsCollection.find(query).toArray();
      res.send(proCollection);
    });
...
app.get("/", async (req, res) => {
  res.send("Elyte Plazza server is running");
});

app.listen(port, () => console.log(`Elyte Plazza running on ${port}`));
